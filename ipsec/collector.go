...
var (
	...
	metricStatus     = prometheus.NewDesc("ipsec_status", "ipsec status of tunnel", []string{"tunnel"}, nil)
	metricBytesIn    = prometheus.NewDesc("ipsec_in_bytes", "received bytes per tunnel", []string{"tunnel"}, nil)
	metricBytesOut   = prometheus.NewDesc("ipsec_out_bytes", "send bytes per tunnel", []string{"tunnel"}, nil)
